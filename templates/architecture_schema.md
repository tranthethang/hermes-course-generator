...
This document defines the exact formatting and structure required for the `architecture.md` file in
a course workspace. The CLI tool uses regex pattern matching to parse this file, so adhering
strictly to this schema is essential.
...
- `Lxx` must be the lesson ID starting with a capital `L` followed by exactly two digits (e.g.
  `L01`, `L02`, `L12`).
...
- `Sxx` must be the section ID starting with a capital `S` followed by exactly two digits (e.g.
  `S01`, `S02`).
- The colon `:` is required.
...
1. **Wrong Level Header Depth:** Using `# Level: begin` or `### Level: begin` instead of
   `## Level: begin`.
...
4. **Nested bullet spacing:** Adding extra indent spaces before the bullet point (e.g., `  - S01:`
   instead of `- S01:`). The section match expects the bullet point to start at the beginning of the
   line or after minimal whitespace.
5. **No Level Prefix:** Declaring lessons before defining a level block. These lessons will not be
   associated with any level and may be ignored or skipped by level-specific status checks.

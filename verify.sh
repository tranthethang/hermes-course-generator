...
if [ -x "$(command -v hermes-course-generator)" ]; then
    echo "✅ CLI tool 'hermes-course-generator' is installed and executable."
else
    echo "❌ CLI tool 'hermes-course-generator' is NOT in PATH or not executable."
    FAILED=1
fi
if [ -d "$HOME/.config/hermes-course-generator/templates" ]; then
    echo "✅ Template folder exists at ~/.config/hermes-course-generator/templates."
    count=$(ls -1 "$HOME/.config/hermes-course-generator/templates" | wc -l)
    echo "   Found $count template files."
else
    echo "❌ Template folder NOT found at ~/.config/hermes-course-generator/templates."
    FAILED=1
fi
for agent in ".gemini/config" ".agents" ".claude"; do
    if [ -f "$HOME/$agent/skills/hermes-course-generator/SKILL.md" ]; then
        echo "✅ Skill registered for: $agent"
    else
        echo "⚠️  Skill NOT registered for: $agent"
    fi
done
if [ $FAILED -eq 0 ]; then
